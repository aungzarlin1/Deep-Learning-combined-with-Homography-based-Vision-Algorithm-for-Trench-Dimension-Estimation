...
def letterbox_image(img, inp_dim):
    ...
    resized_image = cv2.resize(img, (w,h), interpolation = cv2.INTER_CUBIC)
...
def readHomography(homography_file: str) -> "tsHomographyData":
    ...
    pHomograhpyData = tsHomographyData()
    pHomograhpyData.cPoints = 0
    for i in range(4):
        points = fileStorage.getNode("aPoints" + str(i))
        pHomograhpyData.aPoints.append(points.mat())
        pHomograhpyData.cPoints += 1
    pHomograhpyData.matH = fileStorage.getNode("matH").mat()
    pHomograhpyData.widthOut = int(fileStorage.getNode("widthOut").real())
    pHomograhpyData.heightOut = int(fileStorage.getNode("heightOut").real())
    fileStorage.release()
    return pHomograhpyData
